function [polar_DiagL_All, polar_DiagR_All] = separateLinesDiag_H(polar_all,lineZ_ABC)
% for polarH (is horizontal line in image, is verticle line in chart)
...
%     line_ABC = LineABC(p1',p2');
    ...
    if p1(2) < intersect(2) && p2(2) < intersect(2)
%         plot(p1(1),p1(2),'ob');
%         plot(p2(1),p2(2),'ob');
        polar_DiagL = polar_all(i,:);
        polar_DiagL_All = cat(1,polar_DiagL_All,polar_DiagL);
    elseif p1(2) > intersect(2) && p2(2) > intersect(2)
%         plot(p1(1),p1(2),'og');
%         plot(p2(1),p2(2),'og');
