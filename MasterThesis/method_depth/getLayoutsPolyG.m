function [ polyg] = getLayoutsPolyG( vp,h,w )
% getLayoutsPolyG Get sampling box layout.
%
% INPUT:
...
if numL > 0
    for lay=1:size(layout,1)
        ...
        cond1=inpolygon(vp(3,1),vp(3,2),[Ra(1); Rb(1); Rc(1); Rd(1); Ra(1)],[Ra(2); Rb(2); Rc(2); Rd(2);Ra(2)]) & ...
            ...
            ~(inpolygon(vp(1,1),vp(1,2),[Ra(1); Rb(1); Rc(1); Rd(1);Ra(1)],[Ra(2); Rb(2); Rc(2); Rd(2);Ra(2)]));
...
%         cond2=inxs(1) < vp(3,1) & inxs(2) > vp(3,1) & inys(3) < vp(3,2) & inys(4) > vp(3,2);
        cond2=min(inxs) < vp(3,1) & max(inxs) > vp(3,1) & min(inys) < vp(3,2) & max(inys) > vp(3,2);
